...
3. The script always writes the self-contained A4-print-styled HTML intermediate, then drives headless Chromium via **Playwright** (optional dependency — never auto-installed) for the paginated PDF. Every page carries a running head (the note's `title`, or version plus theme) and a foot (`Release note · v{X.Y.Z}` left, page position right), so a page separated from the document still identifies itself. These sit in the page-margin boxes and are a separate document from the report, so design tokens cannot reach them — they are deliberately neutral grey chrome. If Playwright is missing, the HTML is still written and the script fails the PDF step with an explicit message — open the HTML in a browser and print to PDF as the fallback.
   ...
   The whole-tree `diff --shortstat` counts every changed line, so a release whose diff is dominated by generated data seeds, vendored bundles, or plan-execution docs reports a "Lines" figure that is mostly not code. Pass `--exclude-path` (repeatable git pathspec, e.g. `--exclude-path 'docs/**' --exclude-path '*.md'`) to scope that one card to code churn; the card relabels to **Lines (code)** so the reader can tell it is scoped. Commits and active days are unaffected — they describe the release's cadence, not its code volume, so they still cover the whole range regardless of `--exclude-path`.
...
**Output:** `{note-dir}/pdf/{version}-{slug}.pdf` + `.html` (both derived and regenerable — the Markdown note stays the single source of truth; re-render after edits). Override with `--output`; `--html-only` skips the PDF step; `--exclude-path` scopes the recap's Lines card (see above).
