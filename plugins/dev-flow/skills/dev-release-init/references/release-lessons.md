...
| **maven (`pom.xml` present)** | A Java/Kotlin project built with Maven — single module or a multi-module reactor. Applies regardless of app-vs-library: the identity can still be the git tag (deployed app) or the manifest (published library), but either way `pom.xml` itself needs bumping. | `maven` | Release-please's dedicated `pom` updater recursively updates every `pom.xml`'s `/project/version`, and `/project/parent/version` for a submodule inheriting it — so a whole reactor is covered with an empty `extra-files` in the common case. **Lesson 8 does not apply here**: Maven has no checked-in lockfile for `pom.xml` to desync (unlike npm/Python), so freezing the manifest for a deployed app is unnecessary — use `release-type: maven` even when the app is not published. Reach for `extra-files` (`type: xml`, explicit `xpath`) only when a version lives somewhere the `pom` updater doesn't look. |

**Detecting the stack:** presence of `package.json` (node), `pyproject.toml` (python), `Cargo.toml` (rust), `go.mod` (go), `pom.xml` (maven), etc. The stack narrows the *candidate* `release-type`; the **app-vs-library question** decides between `simple` (deployed app) and the ecosystem type (published package) — except Maven, where `maven` is the right `release-type` on both sides of that question. Always ask — the file layout alone cannot tell a deployed Node service from a published npm library.
