...
|---|---|---|
| **User story** | A story is a *need + placeholder for conversation*; a use case is the *worked-out behaviour with all branches*. | Distinct artefacts, relationally bridged: in clew-metamodel projects a story is a **PRD-scoped delivery slice** (`PRD-NNNN.US-NN`, clew ADR-0017) whose optional `Covers: UC-NN` edge points at the use case it exercises. Use-Case 2.0 slices (`UC-NN.S1`…) remain available via the slice mode for teams working use-case-first — but a slice is not the kit's `user_story` artefact. |
...
**The three schools, and the kit's stance.** The literature offers three relationship models: **Cockburn** — different artefacts entirely ("a user story is to a use case as a gazelle is to a gazebo"; [comparison](https://www.viktorcessan.com/whats-the-difference-between-a-user-story-and-a-use-case/)); **Cohn** — stories as cheap conversation placeholders coexisting with use cases as the durable behavioural record ([Mountain Goat](https://www.mountaingoatsoftware.com/articles/advantages-of-user-stories-for-requirements)); **Jacobson** — stories *are* use-case slices, gaining navigability from the use-case parent ([Use-Case 2.0](https://www.ivarjacobson.com/services/use-case-20), [ACM Queue](https://queue.acm.org/detail.cfm?id=2912151)). The kit encodes the Cockburn split with a relational bridge: stories parent to the **PRD** (answering Jacobson's orphan-story complaint via the delivery artefact, not the behavioural one), the `covers` edge links story → use case when both exist, and slice mode serves use-case-first teams. Rationale: coupling stories to use cases typologically would make `spec-prd` unusable without `spec-use-case`, against the per-type opt-in principle (clew ADR-0015/0017).
