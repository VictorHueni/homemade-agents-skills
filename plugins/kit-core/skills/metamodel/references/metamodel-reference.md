...
- the `metamodel` skill's Audit mode — deep metamodel compliance audit: 16 checks covering stack progress, folder placement, internal + external links, ID integrity + cross-references, dependency enforcement, _TODO_ density, mandatory sections, confidence distribution, expiry + staleness, orphaned files, research sync, ADR chains, FBS + epic delivery progress → report at `var/reports/metamodel-audit/`; report-only with proposed fix per finding; run monthly (active dev) or quarterly (maintenance)
- the `metamodel` skill's Migrate mode — one-time migration doctor for repos built before the metamodel: scans any docs/ folder, detects misplaced files using tiered confidence scoring (filename → folder name → content signals), emits atomic fix blocks (git mv + sed link repairs) per file → report at `var/reports/metamodel-migration/`; report-only; run once before the first the `metamodel` skill's Audit mode
...
erDiagram
    ...
    QUALITY_ATTRIBUTES ||--o{ TEST_STRATEGY : "defines tests for"
...
- `qa-*` → the **validate / test** stage, sitting between Step 11 (implementation plans) and deploy/ops (all → `docs/qa/`); it produces the tests that verify the `QA-XXNN` quality requirements `spec-quality-attributes` defines — distinct from it. The lead skill `qa-test-strategy` (was the planned `spec-test-strategy`, OI-0004) **ships**: it covers the test pyramid + coverage + `QA-XXNN`→test-type mapping and **mints `TS-NN`** → `docs/qa/test-strategy.md` (single-collection). *(Still reserved — no skill yet)*: `qa-test-plan`, `qa-acceptance-test` (also covers test execution, results logging, and bug filing, absorbing the dropped `qa-eval-harness` name); `qa-test-scenario` *realises* `UC-NN` (a use case's main + alternate + exception flows become the test scenarios; PRD acceptance criteria are the oracle; step vocabulary from the glossary `GT-NN`)
...
│   └── rcas/
│       └── {YYYY-MM-DD}-{slug}.md
├── qa/                                                  ← Quality assurance & test (test-strategy ships; rest reserved)
│   ├── test-strategy.md                                 ← qa-test-strategy (mints TS-NN)
│   └── (test plans · scenarios/cases · acceptance tests, incl. execution + results + bug filing — planned)
...
| `qa-` | `docs/qa/` | Quality-assurance & test layer (test strategy, test plans, scenarios/cases, acceptance tests incl. execution + results + bug filing). Produces the *tests* that verify the `QA-XXNN` quality requirements `spec-quality-attributes` defines — distinct from it. **`qa-test-strategy` ships** (mints `TS-NN`); `qa-test-scenario` / `qa-test-plan` / `qa-acceptance-test` remain reserved — no skill yet. |
...
| New artefact step, new canonical path | `the `metamodel` skill's `references/modes/audit-check-catalogue.md`` → Check 1 (stack progress paths) |
| New ID format (e.g. new `XX-NN` pattern) | `the `metamodel` skill's `references/modes/audit-check-catalogue.md`` → Check 5 (ID cross-reference regex patterns) |
| New prerequisite dependency in the DAG | `the `metamodel` skill's `references/modes/audit-check-catalogue.md`` → Check 7 (dependency enforcement rules) |
| New artefact step, new canonical path | `the metamodel skill's Migrate mode/references/detection-signals.md` → §Filename patterns + §Folder name patterns + §Content signals |
| New mandatory section in a skill's template | `the `metamodel` skill's `references/modes/audit-check-catalogue.md`` → Check 9 (mandatory sections table) |
| New artefact step, new canonical path | `the metamodel skill's Scaffold mode/references/folder-catalogue.md` → all four variant folder lists (add to Greenfield; add to Brownfield/Strategy/Single-feature if relevant) |
| New artefact step, new canonical path | `the metamodel skill's Scaffold mode/references/index-template.md` → §Detection bash block + §Template stack-progress table (add detection command + row) |
