# Migrate mode — metamodel skill

> Lifecycle mode of the `metamodel` skill (consolidated from the retired standalone migration skill — kit ADR-0007).
> Filename/path detection signals derive from `id_format` + `default_path` in
> `../artefact-types-registry.yaml`; content signals stay curated in `migrate-detection-signals.md`.

You are an expert at scanning existing documentation structures and producing a safe, atomic migration plan to align them with the strategic-architecture metamodel defined in this skill.
...
**Complement to the Audit mode:** run this skill once to migrate structure; then use the Audit mode for ongoing health checks.
...
4. Assemble the report from `migrate-report-template.md`.
...
**When:** a project was built with the v1 canonical paths (nested subfolders like `docs/business/personas/personas.md`) and needs to migrate to the v2 numbered flat structure (e.g. `docs/business/01a-personas.md`) defined in `migrate-path-migration-v2.md`.
...
3. Verify after migration?
   A. Yes — run the Audit mode Mode 1 after apply to confirm zero misplacements
   B. No — skip verification
```

**Process:**
1. **Read `migrate-path-migration-v2.md`** to load the full v1 → v2 mapping table. Every row with `type ≠ no-change` is a migration candidate.
...
4. **For discovery promotion (`type = pattern-c-discovery-promote`)** — the three v1 → v2 transitions defined in `migrate-path-migration-v2.md §Discovery promotion`:
   ...
   - Full bash block is in `migrate-path-migration-v2.md §Discovery promotion` — read that section before emitting the migration script.
...
**If `--apply` (explicit opt-in only):** execute the script, then run the Audit mode Mode 1 to verify zero misplacements. Never apply automatically — always dry-run first.
...
Scan filenames against the patterns in `migrate-detection-signals.md §Filename patterns`. A filename match alone gives confidence Medium; combined with Tier 2 or 3 → High.
...
Check the immediate parent folder name against the patterns in `migrate-detection-signals.md §Folder name patterns`. A folder name match alone gives confidence Low; combined with Tier 1 or 3 → Medium/High.
...
Read only the **first 50 lines** of each file. Check for high-signal section headings from `migrate-detection-signals.md §Content signals`. Never read the full file — this keeps the scan fast on large repos.
...
Full template in `migrate-report-template.md`:
...
| After migration | Run the Audit mode to verify compliance | Complementary skill |
...
| Skill | Relationship |
|---|---|
| the Audit mode | Run AFTER migration to verify ongoing compliance. Audit assumes metamodel compliance; migration doctor assumes pre-compliance. |
...
| this skill | The canonical source for all detection rules in migrate-detection-signals.md |
...
- **`migrate-detection-signals.md`** — the complete tiered detection ruleset: filename patterns, folder name patterns, content signals, outside-scope patterns. Update this file when the metamodel gains new artefacts.
- **`migrate-report-template.md`** — full migration report skeleton with atomic fix block format.
- **`migrate-methodology.md`** — rationale for tiered detection, inbound link tracking, and the report-only discipline.
...
6. **Next step** — "After applying all blocks, run the Audit mode Mode 1 to verify compliance."
