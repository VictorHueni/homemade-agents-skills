...
**Dependency drift** — the gap between what a document claims its prerequisites are and what actually exists — silently corrupts the traceability chain. A PRD referencing `E-04` when `E-04` doesn't exist in the epic catalogue is a ghost dependency. The audit enforces the DAG defined in this skill.
...
|---|---|
| §1 Stack progress | this skill — canonical 11-step build order |
| §2 Folder placement | this skill — canonical output paths table |
...
| §7 Dependency enforcement | this skill DAG hard rules |
...
| §17 Frontmatter validity | this skill's `references/artefact-frontmatter.md` — canonical OKF-superset schema (6 OKF fields incl. required `type`, + kit lifecycle fields) + conditional supersession fields + OKF reserved-file handling (`index.md`/`log.md`, root `okf_version`); staleness depends on `last_reviewed` + `review_interval` |
| §18 Open items governance | this skill's `references/open-items-governance.md` — single source of truth for the central ledger as sole authoring surface, taxonomy, lifecycle, and schema. Schema compliance enforces §4. Source-location provenance enforces §4 (`Source anchor` + `Source heading` is the provenance pair, filed directly at row-creation time). Closure drift enforces §3 (closure must be evidenced via `Tracker ref`). Stale open items surface §3 lifecycle staleness without auto-closing — re-triage stays operator-driven via `util-open-items` |
