...
**Dependency drift** — the gap between what a document claims its prerequisites are and what actually exists — silently corrupts the traceability chain. A PRD referencing `E-04` when `E-04` doesn't exist in the epic catalogue is a ghost dependency. The audit enforces the DAG defined in the `metamodel` skill.
...
|---|---|
| §1 Stack progress | the `metamodel` skill — canonical 11-step build order |
| §2 Folder placement | the `metamodel` skill — canonical output paths table |
...
| §7 Dependency enforcement | the `metamodel` skill DAG hard rules |
...
| §17 Frontmatter validity | the `metamodel` skill's `references/artefact-frontmatter.md` — canonical OKF-superset schema (6 OKF fields incl. required `type`, + kit lifecycle fields) + conditional supersession fields + OKF reserved-file handling (`index.md`/`log.md`, root `okf_version`); staleness depends on `last_reviewed` + `review_interval` |
| §18 Open items governance | the `metamodel` skill's `references/open-items-governance.md` — single source of truth for the central ledger as sole authoring surface, taxonomy, lifecycle, and schema. Schema compliance enforces §4. Source-location provenance enforces §4 (`Source anchor` + `Source heading` is the provenance pair, filed directly at row-creation time). Closure drift enforces §3 (closure must be evidenced via `Tracker ref`). Stale open items surface §3 lifecycle staleness without auto-closing — re-triage stays operator-driven via `util-open-items` |
