...
`homemade-claude-kit` is a personal Claude Code skill/plugin toolkit — 56 skills across 10 toggleable plugin sets (strategic-architecture docs, developer workflow, agent-execution) — installed into other projects via `install.sh` (symlink channel) or consumed directly as a Claude Code marketplace (`.claude-plugin/marketplace.json`). The repo also dogfoods its own governance system: its own backlog runs on the `github` open-items backend (see Domain context below).
...
- **Build order:** the `metamodel` skill — 16 steps (Vision → Implementation plans). This repo does not run its own spine (it's the kit, not a product built through it) — see the note at the top of `docs/index.md`. The live surfaces here are the open-items ledger and the use-case registry, both indexed.
...
```bash
# Install / sync skills+commands+rules into ~/.claude, ~/.codex, ~/.agents
./install.sh
...
# Auto-fix missing/invalid skill frontmatter fields
./scripts/fix-skills-frontmatter.sh
