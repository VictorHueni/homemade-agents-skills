...
## A remote HTTP MCP server can reject its own cached OAuth client — "Unrecognized client_id"

A third drift mode, distinct from both above: a remote MCP server reached via OAuth (`"type": "http"` pointing at the provider's own domain, e.g. `mcp.supabase.com`) does Dynamic Client Registration once, then Claude Code caches that `client_id` and replays it on every subsequent `/mcp` reconnect or session start. If the provider's OAuth server later loses or invalidates that registration server-side, every replay fails at the very first hop — before any login/consent screen — with `{"message":"Unrecognized client_id"}`. Nothing local is wrong: the plugin can be on its latest cached version, the marketplace can be freshly refreshed, `.mcp.json` can be untouched. `claude mcp list` shows the server as `! Needs authentication` with no further detail.

**Confirmed widespread, not project-specific:** this exact error and mechanism has been independently reported for Supabase's MCP server across Claude Code, Cursor, and OpenCode (e.g. `anthropics/claude-code#84609`, `supabase/supabase#43662`/`#43663`) — it is a break in the provider's stored client registrations, not a per-user misconfiguration.

**How to apply:** `claude mcp logout <server-name>` clears the locally cached OAuth credential/client registration, forcing a fresh Dynamic Client Registration on the next login rather than replaying the dead `client_id`. Then reconnect from an **interactive terminal** — `claude mcp login <server-name>` fails outright over a non-interactive shell (`stdin isn't a terminal`) since it needs to open a browser and receive the redirect; drive it via `/mcp` in the interactive session instead. If the fresh registration still gets `Unrecognized client_id`, the break is server-side and matches the known open issues above — logout/login cannot fix it locally; check those trackers for status before spending more time on local config. For unblocking real work meanwhile, fall back to the provider's own CLI or Management API with a personal access token, same as the REST fallback above.
